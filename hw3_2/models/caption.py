...
class Caption(nn.Module):
    ...
    def forward(self, samples, target, target_mask):
        ...
        hs,attention_weight = self.transformer(self.input_proj(src), mask,
...
def build_model_out(config):
    backbone = build_backbone(config)
    transformer = build_transformer(config)

    model = Caption_out(backbone, transformer, config.hidden_dim, config.vocab_size)
    criterion = torch.nn.CrossEntropyLoss()
