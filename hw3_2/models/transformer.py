...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, pos_embed, tgt, tgt_mask):
        ...
        hs,attention_weight = self.decoder(tgt, memory, memory_key_padding_mask=mask, tgt_key_padding_mask=tgt_mask,
                          pos=pos_embed, query_pos=query_embed,
                          tgt_mask=generate_square_subsequent_mask(len(tgt)).to(tgt.device))
        # print(hs.shape,attention_weight.shape)

        return hs,attention_weight
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_pre(self, tgt, memory,
                    ...
                    query_pos: Optional[Tensor] = None):
        tgt2 = self.norm1(tgt)
        # print(tgt2.shape,query_pos.shape)
